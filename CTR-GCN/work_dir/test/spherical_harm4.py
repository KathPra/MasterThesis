...
class symmetry_module(nn.Module):
    ...
    def azimuth(self,x):
        ...
        spine_vec = x[:,:,:,20]-x[:,:,:,0] 
        norm_spine = (spine_vec / (LA.norm(spine_vec, dim=1).unsqueeze(1))).unsqueeze(3) # N, C,T,1

        #norm_spine = norm_spine.permute(0,2,1).view(N*T,C)
        ...
        norm_vec = vec / (LA.norm(vec, dim=1).unsqueeze(1)) # N, C,T,V
        
        #norm_vec = norm_vec.permute(0,2,1,3).contiguous().view(N*T,C,V)
        

        # compute angle between vectors: theta = cos^-1 [(a @ b) / |a|*|b|] -> a,b are normalized, thus |a|=|b|=1 -> theta = cos^-1 [(a @ b)]
        #angle = torch.acos(norm_spine @ norm_vec)
        # angle = torch.einsum("ijkl,ijkm->ijlm", norm_spine, norm_vec)
        # angle = angle.view(N,T,C,V).permute(0,2,1,3)
        # dot product not yet possible

        raise ValueError(norm_spine.shape, vec.shape, norm_vec.shape)
    ...
    def forward(self, x):
        ...
        raise ValueError(pol_input.shape, r.shape, theta.shape, phi.shape)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.sym = symmetry_module(64,16)
