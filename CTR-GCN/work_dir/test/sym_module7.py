...
class unit_gcn(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_subset):
            ...
            A1 = A[i]                   # 25 x 25
            A2 = x.view(N, C * T, V)    # 128, 3*64=192, 25
            z = self.conv_d[i](torch.matmul(A2, A1).view(N, C, T, V))
            # raise ValueError(A2.shape, A1.shape, z.shape)
...
class symmetry_module(nn.Module):
    def __init__(self, in_channels, out_channels, stride = 1):
        ...
        # self.fnn = nn.Linear(in_channels,in_channels)

        self.emb2 = nn.Sequential(
            nn.MaxPool2d(1, stride=1)
            #nn.Tanh(), # try 1 d
            #nn.Conv2d(in_channels, 1, kernel_size = 1, stride= stride),
            )
    ...
    def calc_mape(self, actual, predic):
        diff = actual - predic
        rel_diff = diff / (actual+0.0001)
        abso = torch.abs(rel_diff)
        mean = torch.mean(abso, dim = 2) # mean over time
        return mean
    ...
    def forward(self,x):
        ...
        for v in range(x.size(3)):
            vector = x[:,:,:,v]#.unsqueeze(3)
            matrix = torch.stack([vector] * dup, axis=3)
            remaining = x
            #raise ValueError(x.shape, vector.shape, matrix.shape, remaining.shape)
            res = self.calc_mape(matrix, remaining).unsqueeze(3) # N x C x T x Joint
            
            sym = torch.cat((sym,res), dim = 3) if sym is not None else res
        
        # remove self-similarity
        ind = np.diag_indices(sym.shape[2]) # [128, 1, 25, 25]
        sym[:,:,ind[0], ind[1]] = torch.zeros(sym.shape[2]).cuda(x.get_device())
    
        res, _ = torch.max(sym, dim =1, keepdim = True)
        #res = self.emb2(sym)

        #raise ValueError(res[0,0,:5,0:5]) # similarity matrix 
      
        #raise ValueError(res.shape)
        ...
        # f = f.permute(0,3,1,2).contiguous() # N,T,C,V
        # f = self.emb2(f) # decrease time dim
        
        # f = f.permute(0,2,1,3).contiguous() # N,C,T,V -> 64 channels
...
class final_TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x,f):
        y = self.relu(self.tcn1(self.gcn1(x)) + self.residual(x))
        y = torch.matmul(y.unsqueeze(1), f.unsqueeze(2))
        raise ValueError(y.shape, f.shape, x.shape)
        y = y+f
        y = self.embed(y)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.sym = symmetry_module(64,256)
