...
class four_unit_tcn(nn.Module):
    ...
    def forward(self, x):
        ...
        fourier_2 = torch.fft.fft(fourier_input,dim=1) # default "backwards"
        fourier_2 = fourier_2.abs() # retrieve magnitude
        fourier_2 = fourier_2[:,1:,:,:]
        first= fourier_2.new(N, 1,fourier_2.size(2), V).zero_()
        fourier_2 = torch.cat([fourier_2, first], dim=1)
