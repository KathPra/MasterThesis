...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.data_bn = nn.BatchNorm1d(num_person * 2 * num_point)
    ...
    def forward(self, x):
        ...
        x = sym.view(N,M,2,T,V).permute(0, 1, 4, 2, 3).contiguous().view(N, M * V * 2, T)
        #raise ValueError(x.shape)
        # order is now N,(M,V,C),T
        #print(x.shape) -> 64, 150, 64
        x = self.data_bn(x)
        #print(x.shape) -> shape stays the same
        x = x.view(N, M, V, 2, T).permute(0, 1, 3, 4, 2).contiguous().view(N * M, 2, T, V)
