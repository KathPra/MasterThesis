...
class four_unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, adaptive=True):
        super(four_unit_gcn, self).__init__()
        self.out_c = out_channels
        self.in_c = in_channels
        self.num_subset = A.shape[0]
        self.adaptive = adaptive
    ...
    def forward(self, x):
        ...
        cum_vel = torch.cumsum(vel,3)

        # fourier
        fourier_input = torch.clone(cum_vel)
        fourier_2 = torch.fft.fft(fourier_input,dim=1)
        ...
        for i in range(self.num_subset):
            A1 = A[i]
            A2 = x.view(N, C *2 * T, V)
            z = self.conv_d[i](torch.matmul(A2, A1).view(N, C*2, T, V))
            y = z + y if y is not None else z
...
class four_TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True):
        ...
        self.gcn1 = four_unit_gcn(in_channels, out_channels, A, adaptive=adaptive)
        self.tcn1 = unit_tcn(out_channels, out_channels, stride=stride)
        self.relu = nn.ReLU(inplace=True)
        if not residual:
            self.residual = lambda x: 0
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l1 = four_TCN_GCN_unit(6, 64, A, residual=False, adaptive=adaptive)
