...
class four_unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1):
        ...
        pad = int((kernel_size - 1) / 2)
        self.conv = nn.Conv2d(90, out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0),
                              stride=(stride, 1))
    ...
    def forward(self, x):
        ...
        # window = 8
        # for i in range(fourier_input.size(2)):
        #     i +=1
        #     if i % 8 == 0:
        #         fourier_1 = fourier_input[:,i-window:i,:,:]
        #         fourier_1 = torch.fft.rfft(fourier_1,dim=1)
        #         fourier_1 = fourier_1.abs() # retrieve magnitude
        #         fourier_1 = fourier_1[:,1:,:,:]
        #         x = torch.cat([x,fourier_1], dim =1)
