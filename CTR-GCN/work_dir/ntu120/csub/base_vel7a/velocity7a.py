...
class vel_unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, adaptive=True):
        ...
        self.conv_d = nn.ModuleList()
        for i in range(self.num_subset):
            self.conv_d.append(nn.Conv2d(in_channels+1, out_channels, 1))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_subset):
            A1 = A[i]
            A2 = x.view(N, (C+1) * T, V)
            z = self.conv_d[i](torch.matmul(A2, A1).view(N, (C+1), T, V))
            y = z + y if y is not None else z
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l1 = vel_TCN_GCN_unit(3, 64, A, residual=False, adaptive=adaptive) # 3 -> 4
