...
class four_unit_tcn(nn.Module):
    ...
    def forward(self, x):
        ...
        fourier_2 = torch.fft.rfft(fourier_input,dim=1) # own re-scaling
        fourier_2 = torch.log(fourier_2+0.01)
        ...
        norm_f = norm_f.unsqueeze(1)        
        fourier_norm = fourier_2 / norm_f
        

        x = torch.cat([x,fourier_norm], dim =1 )
        x = self.bn(self.conv(x))
        return x
