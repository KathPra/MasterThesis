...
class four_unit_tcn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([x1,fourier_2], dim =2)
        #raise ValueError(x.shape, x1.shape, fourier_2.shape)
        # (torch.Size([128, 64, 128, 25]), torch.Size([128, 64, 64, 25]), torch.Size([128, 64, 64, 25]))
        return x
...
class after_four_TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x):
        ...
        #ValueError: torch.Size([128, 64, 128, 25])
        x1 = x[:,:,0:64,:].contiguous()
        #raise ValueError(x.shape)
        #ValueError: torch.Size([128, 64, 64, 25])
        y = self.relu(self.tcn1(self.gcn1(x1)) + self.residual(x1))
        return y
