...
class Model(nn.Module):
    ...
    def lin_trans_angle(self,x):
        ...
        # Transform angle with gaussian kernel
        # transform_z = T.functional.gaussian_blur(kernel_size=(1, 5), sigma=(0.1, 0.2))
        # transform_y = T.functional.gaussian_blur(kernel_size=(1, 5), sigma=(0.1, 0.2))
   

        #raise ValueError(x_rotzy[0,:,0], norm_vec.shape, rot_z.shape,x_rotzy.shape)
        # rotz = transform_z(rot_z)
        # roty = transform_y(rot_y)
        # # plot vectors
        # fig = plt.figure()
        # ax = fig.add_subplot(111, projection = "3d")
        # ax.set_xlim = ([-1,1])
        ...
        # ax.set_zlim = ([-1,1])
        # start = [0,0,0]
        # ax.quiver(start[0], start[1], start[2],norm_vec[0,0].cpu().detach().numpy(), norm_vec[0,1].cpu().detach().numpy(), norm_vec[0,2].cpu().detach().numpy())
        # ax.quiver(start[0], start[1], start[2],norm_vec[0,0].cpu().detach().numpy(), norm_vec[0,1].cpu().detach().numpy(), norm_vec[0,2].cpu().detach().numpy())
        # ax.quiver(start[0], start[1], start[2],norm_vec[0,0].cpu().detach().numpy(), norm_vec[0,1].cpu().detach().numpy(), norm_vec[0,2].cpu().detach().numpy())
        # plt.savefig("testejflkdsgfng")
        return rot_z, rot_y
        bskel_rot = rot_z @ x
        bskel_rot1 = rot_y @ bskel_rot
        bskel_rot1 = bskel_rot1.unsqueeze(2)
        # when input contain only zeros -> rotation will return NAN values. This returns them back into zeros.
        bskel_rot1 = torch.nan_to_num(bskel_rot1, nan=0.) 
        if torch.isnan(bskel_rot1).any():
            for i in range(x.size(0)):
                if torch.isnan(bskel_rot[i]).any():
                    c = torch.isnan(bskel_rot[i])   
                    check = bskel_rot[i,c]
                    org = x[i]
                    org1 = bskel_rot[i]
                    raise ValueError("NaN found:", i, check, x[i], rot_z[i], rot_y[i], spine_vec[i], norm_vec[i])
    ...
    def forward(self, x):
        ...
        #raise ValueError(x_rot.shape, rot1.shape, rot2.shape, x_rot[0,:,0,0])

        ## Plot
        # self.plot(0, x_rot, dim = 4, string1="Rotated")
        # self.plot(5, x_rot, dim = 4, string1="Rotated")
        # self.plot(15, x_rot, dim = 4, string1="Rotated")
        # self.plot(25, x_rot, dim = 4, string1="Rotated")
        # self.plot(35, x_rot, dim = 4, string1="Rotated")
        # self.plot(45, x_rot, dim = 4, string1="Rotated")
        # self.plot(55, x_rot, dim = 4, string1="Rotated")
        # self.plot(63, x_rot, dim = 4, string1="Rotated")

        raise ValueError(x_rot[0,:,0,:])
