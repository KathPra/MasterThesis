...
class symmetry_module(nn.Module):
    ...
    def Spherical(self,x):
        ...
        theta = torch.atan2(torch.sqrt(xy)+0.00001, x[:, 2]+0.00001)  # for elevation angle defined from Z-axis down -> INTERESTING
        # ptsnew[:,4] = np.arctan2(xyz[:,2], np.sqrt(xy)) # for elevation angle defined from XY-plane up
        phi = torch.atan2(x[:, 1]+0.00001, x[:, 0]+0.00001)
        return r, theta, phi
    ...
    def forward(self, x):
        ...
        r, theta, phi = self.Spherical(pol_input)
