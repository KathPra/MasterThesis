...
class fourier_module(nn.Module):
    def __init__(self):
        super(fourier_module, self).__init__()
    ...
    def forward(self, x, l):
        ...
        fourier_2 = torch.fft.fft(x,dim=1)
        fourier_2 = fourier_2.abs() # retrieve magnitude
        fourier_2 = fourier_2[:,1:,:,:,:]
        
        N,_ , T, V,_ = x.size()
        x = fourier_2.permute(0,1,4,2,3).contiguous().view(N,-1,T,V).cuda(x.get_device())
        return x
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        small_motion_joints = [21,22,23,24]
        for i in range(V):
