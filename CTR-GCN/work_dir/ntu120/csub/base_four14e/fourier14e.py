...
class four_unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0),
                              stride=(stride, 1))
        self.convf = nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0,
                              stride=(stride, 1))
...
class unit_fgcn(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_subset):
            A1 = A[i]
            A2 = f.view(N, C * T, V)
            zf = self.conv_d[i](torch.matmul(A2, A1).view(N, C, T, V))
            yf = zf + yf if yf is not None else zf

        y1 = y + yf

        y1 = self.bn(y1)
        y1 += self.down(x) # residual connection
        y1 = self.relu(y1)
