...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, num_set=3, seg = 64):
        ...
        #if args.train:
        self.spa = self.one_hot(self.N, num_point, self.seg)
        self.spa = self.spa.permute(0, 3, 2, 1).cuda()
        self.tem = self.one_hot(self.N, self.seg, num_point)
        ...
        self.cnn = local(self.dim1, self.dim1 * 2, bias=True)
        self.compute_g1 = compute_g_spa(self.dim1 // 2, self.dim1, bias=True)
        self.gcn1 = gcn_spa(self.dim1 // 2, self.dim1 // 2, bias=True)
        self.gcn2 = gcn_spa(self.dim1 // 2, self.dim1, bias=True)
        self.gcn3 = gcn_spa(self.dim1, self.dim1, bias=True)
    ...
    def forward(self, x):
        ...
        dif0 = torch.clone(dif)
        dif0 = self.dif_embed(dif0)
