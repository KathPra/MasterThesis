...
class symmetry_module(nn.Module):
    def __init__(self, in_channels, out_channels, stride = 1):
        ...
        self.emb3 = nn.Sequential(
            ...
            ## convert euler coordinates (3D coordinates) to cylindrical coordinates
    def Cylindrical(self,x):
        ptsnew = torch.zeros(x.shape).cuda(x.device)
        xy = x[:, 0] ** 2 + x[:, 1] ** 2
        ptsnew[:, 0] = torch.sqrt(xy + x[:, 2] ** 2)
        ptsnew[:, 1] = torch.atan2(x[:, 1], x[:, 0]) 
        ptsnew[:, 2] = x[:, 2]
        return ptsnew
    ...
    def forward(self, x):
        ...
        cyl_input = x.view(N,-1)
        cyl = self.Cylindrical(cyl_input)
        x =cyl.view(N,C,T,V)
