...
class four_unit_tcn(nn.Module):
    ...
    def forward(self, x):
        ...
        fourier_input = fourier_input.permute(0,3,1,2).reshape(N*V,C,T) # over time, joints are independent
        fourier_2 = torch.fft.rfft2(fourier_input) # last 2 dims
        fourier_2 = fourier_2.abs() # retrieve magnitude
        fourier_2 = fourier_2.reshape(N,V,-1,T).permute(0,2,3,1)
