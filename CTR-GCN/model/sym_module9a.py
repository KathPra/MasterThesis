...
class symmetry_module(nn.Module):
    def __init__(self, in_channels, out_channels, stride = 1):
        ...
        self.fnn = nn.Linear(self.joint_channels,self.joint_channels)

        self.emb2 = nn.Sequential(
            nn.BatchNorm2d(in_channels), # try 1 d
            nn.Conv2d(in_channels, in_channels, kernel_size = (1,3), stride= stride, padding =(0,1)),
            nn.ReLU(),
        ...
        self.emb3 = nn.Sequential(
            nn.BatchNorm2d(in_channels), # try 1 d
            nn.Conv2d(in_channels, self.time_channels, kernel_size = 1, stride= stride),
            nn.ReLU(),
        )

    def compute_vel(self,x):
        N, C, T, V = x.size()
        vel = x.clone()
...
class final_TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True):
        ...
        self.tcn1 = unit_tcn(out_channels, out_channels, stride=stride)
        self.relu = nn.ReLU(inplace=True)
        self.embed = nn.Conv2d(in_channels+64, out_channels, 1)
        if not residual:
            self.residual = lambda x: 0
