...
class vel_unit_gcn(nn.Module):
    ...
    def forward(self, x):
        ...
        cum_vel = torch.cumsum(vel,3) # cumulative sum over joint
        
        x = torch.cat([x, cum_vel],dim=1)
...
class TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x):
        ...
        # RuntimeError: Given groups=1, weight of size [64, 64, 5, 1], expected input[128, 67, 64, 25] to have 64 channels, but got 67 channels instead
        return y
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, num_set=6):
        ...
        self.l1 = vel_TCN_GCN_unit(6, 64, A, residual=False, adaptive=adaptive) # 3 -> 6
