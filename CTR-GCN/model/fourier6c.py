...
class four_unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1):
        ...
        pad = int((kernel_size - 1) / 2)
        self.conv = nn.Conv2d(96, out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0),
                              stride=(stride, 1))
    ...
    def forward(self, x):
        ...
        fourier_2 = torch.fft.rfft(fourier_input,dim=2)
        ...
        fourier_norm = fourier_2 / norm

        input = fourier_norm[0,0,:,5].cpu().detach().numpy()
        input1 = fourier_norm[10,0,:,5].cpu().detach().numpy()
        input2 = fourier_norm[30,0,:,5].cpu().detach().numpy()
        input3 = fourier_norm[17,0,:,5].cpu().detach().numpy()
        plt.plot(input1)#, input1)
        plt.plot(input)#, input1)
        plt.plot(input2)#, input1)
        plt.plot(input3)#, input1)
        plt.savefig('fourier_6c_norm_dim2_1_rfft.png')
        plt.close()
        raise ValueError(fourier_norm.shape)

        x = torch.cat([x,fourier_2], dim =1 )
        x = self.bn(self.conv(x))
        return x
