...
class four_unit_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1):
        ...
        pad = int((kernel_size - 1) / 2)
        self.conv = nn.Conv2d(in_channels+4, out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0),
                              stride=(stride, 1))
    ...
    def forward(self, x):
        ...
        for i in range(8):
            ...
            fourier_input = fourier_input[:,:,start:end,:]
            fourier_1 = torch.fft.rfft(fourier_input,dim=1, n = window)
            fourier_1 = fourier_1.abs() # retrieve magnitude
            fourier_1 = fourier_1[:,1:,:,:]
            fourier = torch.cat([fourier, fourier_1], dim = 2) if fourier is not None else fourier_1
            raise ValueError(fourier.shape, fourier_1.shape, fourier_input.shape)
