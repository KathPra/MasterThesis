...
class four_unit_tcn(nn.Module):
    ...
    def forward(self, x):
        ...
        # Legs
        fourier_ll = fourier_input[:,:, :, 12:16]
        fourier_ll = torch.fft.rfft(fourier_ll,dim=1)
        fourier_ll = fourier_ll.abs()
        fourier_ll = fourier_ll[:,1:,:,:]
        fourier_lr = fourier_input[:,:,:, 16:20]
        fourier_lr = torch.fft.rfft(fourier_lr,dim=1)
        fourier_lr= fourier_lr.abs()
        fourier_lr = fourier_lr[:,1:,:,:]

        arm_dif = fourier_lr-fourier_ll

        raise ValueError(arm_dif[0,:,:,:] )
        x = torch.cat([x,arm_dif], dim = 1)  
        x = self.bn(self.conv(x))
        return x
