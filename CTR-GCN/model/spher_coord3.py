...
class symmetry_module(nn.Module):
    ...
    def azimuth(self,x):
        eps = 0.0000001
        azimuth = torch.atan2(x[:, 1]+eps, x[:, 0]+eps)
    ...
    def forward(self, x, x_rot):
        ...
        azimuth = self.azimuth(x_rot) # input [128,3,64,25], output [128, 64, 25]
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x_prep = x.permute(0,2,1,3).contiguous().view(N*M*T,C,V)
        x_rot_half = rot1 @ x_prep
