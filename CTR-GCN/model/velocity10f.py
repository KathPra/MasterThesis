...
class vel_TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True):
        ...
        self.gcn1 = vel_unit_gcn(in_channels, out_channels, A, adaptive=adaptive)
        self.vel_tcn1 = unit_tcn(out_channels, out_channels, stride=stride)
        self.relu = nn.ReLU(inplace=True)
        if not residual:
            self.residual = lambda x: 0
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l1 = vel_TCN_GCN_unit(6, 64, A, residual=False, adaptive=adaptive) 
