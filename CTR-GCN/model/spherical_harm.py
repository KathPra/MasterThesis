...
class symmetry_module(nn.Module):
    ...
    def Spherical(self,x):
        ...
        ptsnew[:, 1] = torch.atan2(torch.sqrt(xy), x[:, 2])  # for elevation angle defined from Z-axis down
        # ptsnew[:,4] = np.arctan2(xyz[:,2], np.sqrt(xy)) # for elevation angle defined from XY-plane up
        ptsnew[:, 2] = torch.atan2(x[:, 1], x[:, 0])
        return ptsnew
    ...
    def forward(self, x):
        ...
        pol_input = x.view(N,-1)
        pol = self.Spherical(pol_input)
        x = pol.view(N,C,T,V)
