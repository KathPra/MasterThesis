...
class symmetry_module(nn.Module):
    ...
    def azimuth(self,x):
        ...
        #raise ValueError(angle[0,:,:])#, norm_spine[0,:,:], norm_vec[0,:,0])
...
# if torch.isnan(angle).any():
        ...
        #         if torch.isnan(angle[i]).any():
        #             for c in range(x.size(1)):
        #                 if torch.isnan(angle[i,c]).any():
        #                     for t in range(x.size(2)):
        #                         if torch.isnan(angle[i,c,t]).any():
        #                             n = torch.isnan(angle[i,c,t])   
        #                             check = angle[i,c,t]

        #                             raise ValueError("NaN found:", i,c,t, x[i,c,t],angle[i,c,t])
    ...
    def forward(self, x):
        ...
        azimuth = azimuth.permute(0,2,1) # N * V * T
        #raise ValueError(azimuth.shape)
        order = self.fnn(azimuth).permute(0,2,1).unsqueeze(1)
        sym = self.emb1(order)
        sym = self.emb2(sym)
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x_rot.permute(0, 3, 1, 2).contiguous().view(N, V * C, T)
