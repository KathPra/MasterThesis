...
class four_unit_tcn(nn.Module):
    ...
    def forward(self, x):
        ...
        fourier1d = torch.fft.rfft(fourier_input,dim=2)
        fourier_mag = fourier1d.abs() # retrieve magnitude
        fourier_phase = fourier1d.angle() #retrieve phase
        #fourier_mag = fourier_mag[:,:,1:,:]

        x = torch.cat((fourier_mag, fourier_phase), dim = 1)
        x = self.bn(self.conv(x))
        return x
